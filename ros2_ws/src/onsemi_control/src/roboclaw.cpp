...
//id  RoboClaw::roboSetup(const TPIDQ m1Pid, const TPIDQ m2Pid, float m1MaxCurrent,
RoboClaw::RoboClaw(const TPIDQ m1Pid, const TPIDQ m2Pid, float m1MaxCurrent,
    ...
    if (openPort(device_name) == 0){
      ...
      SetEncoder(kSETM1ENCODER,0);
      SetEncoder(kSETM2ENCODER,0);

      // onsemi_control:srv::ResetEncoders::Request resetRequest;
      // resetRequest.left = 0;
      // resetRequest.right = 0;
      // onsemi_control::srv::ResetEncoders::Response response;
      // resetEncoders(resetRequest, response);
      g_singleton = this;
      readSensorGroup();
    } else{
      RCUTILS_LOG_INFO("[RoboClaw::RoboClaw] Cannot Open RoboClaw : %s",  device_name.c_str());
    }
}
...
void RoboClaw::doMixedSpeedAccel(uint32_t accel_quad_pulses_per_second,
                                     ...
                                     int32_t m2_quad_pulses_per_second) {
  if (singleton() != nullptr) {
    writeN(false, 15, portAddress_, kMIXEDSPEEDACCEL,
          SetDWORDval(accel_quad_pulses_per_second),
...
void RoboClaw::doM1SpeedAccel(uint32_t accel_quad_pulses_per_second,
                                     int32_t m1_quad_pulses_per_second) {
  if (singleton() != nullptr) {
    writeN(true, 11, portAddress_, kM1SPEEDACCEL,
          SetDWORDval(accel_quad_pulses_per_second),
          SetDWORDval(m1_quad_pulses_per_second),
          1 /* Cancel any previous command. */
    );
...
void RoboClaw::doMixedDuty(uint8_t m1, int8_t m2 ) {
  if (singleton() != nullptr) {

    writeN(true, 6, portAddress_, kMIXEDDUTY, m1, 0, m2, 
          1 /* Cancel any previous command. */
    );
  }
}
...
int RoboClaw::openPort(std::string device_name) {

  RCUTILS_LOG_INFO("[RoboClaw::openPort] about to open port: %s", device_name.c_str());
  device_port_ = open(device_name.c_str(), O_RDWR | O_NOCTTY | O_SYNC);
  if (device_port_ < 0) {
    RCUTILS_LOG_ERROR( "[RoboClaw::openPort] Unable to open USB port: %s, errno: (%d) %s", device_name.c_str(), errno, strerror(errno));
    return 1;
  ...
  if (ret < 0) {
    RCUTILS_LOG_ERROR( "[RoboClaw::openPort] Unable to get terminal options (tcgetattr), error: %d: %s", errno, strerror(errno));
  ...
  if (cfsetispeed(&portOptions, B38400) < 0) {
    RCUTILS_LOG_ERROR( "[RoboClaw::openPort] Unable to set terminal speed (cfsetispeed)");
    throw new TRoboClawException( "[RoboClaw::openPort] Unable to set terminal speed (cfsetispeed)");
  }

  if (cfsetospeed(&portOptions, B38400) < 0) {
    RCUTILS_LOG_ERROR( "[RoboClaw::openPort] Unable to set terminal speed (cfsetospeed)");
    throw new TRoboClawException( "[RoboClaw::openPort] Unable to set terminal speed  (cfsetospeed)");
  }

  // Now that we've populated our options structure, let's push it back to the
  // system.
  if (tcsetattr(device_port_, TCSANOW, &portOptions) < 0) {
    RCUTILS_LOG_ERROR( "[RoboClaw::openPort] Unable to set terminal options (tcsetattr)");
    throw new TRoboClawException( "[RoboClaw::openPort] Unable to set terminal options (tcsetattr)");
  }
  return 0;
}
...
uint8_t RoboClaw::readByteWithTimeout() {
  ...
  if (retval < 0) {
    RCUTILS_LOG_ERROR("[RoboClaw::readByteWithTimeout] Poll failed (%d) %s", errno, strerror(errno));
    throw new TRoboClawException("[RoboClaw::readByteWithTimeout] Read error");
  } else if (retval == 0) {
    std::stringstream ev;
    ev << "[RoboClaw::readByteWithTimeout] TIMEOUT revents: " << std::hex << ufd[0].revents;
    ...
    restartPort();
    throw new TRoboClawException( "[RoboClaw::readByteWithTimeout] Error on socket");
  } else if (ufd[0].revents & POLLIN) {
    unsigned char buffer[1];
    ssize_t bytesRead = ::read(device_port_, buffer, sizeof(buffer));
    if (bytesRead != 1) {
      RCUTILS_LOG_ERROR( "[RoboClaw::readByteWithTimeout] Failed to read 1 byte, read: "
          "%d",
          (int)bytesRead);
...
void RoboClaw::readSensorGroup() {
  ...
  if (singleton() != nullptr) {
      ...
      g_sensor_value_group_.logic_battery_level = singleton()->cache_getLogicBatteryLevel();
      g_sensor_value_group_.m1_encoder = singleton()->cache_getM1Encoder();
      g_sensor_value_group_.m1_encoder_command_result = singleton()->cache_getEncoderCommandResult(kGETM1ENC);
      g_sensor_value_group_.m1_pidq = singleton()->cache_getPIDQ(kGETM1PID);
      g_sensor_value_group_.m1_velocity = singleton()->cache_getVelocity(RoboClaw::kGETM1SPEED);
      g_sensor_value_group_.m2_encoder = singleton()->cache_getM2Encoder();
      g_sensor_value_group_.m2_encoder_command_result = singleton()->cache_getEncoderCommandResult(kGETM2ENC);
      g_sensor_value_group_.m1_pidq = singleton()->cache_getPIDQ(kGETM2PID);
      g_sensor_value_group_.m2_speed = singleton()->cache_getVelocity(RoboClaw::kGETM2SPEED);
      g_sensor_value_group_.main_battery_level = singleton()->cache_getMainBatteryLevel();

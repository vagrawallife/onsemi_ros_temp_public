...
class RoboClaw {
 public:
  ...
  ~RoboClaw();
  // Constructor.
  //void roboSetup(const TPIDQ m1Pid, const TPIDQ m2Pid, float m1MaxCurrent,
  ...
  void doM1SpeedAccel(uint32_t accel_quad_pulses_per_second,
              int32_t m1_quad_pulses_per_second);

  void doMixedDuty(uint8_t m1, int8_t m2 );          
 ...
 private:
  ...
  int openPort(std::string device_name);
  ...
  // Reset the encoders.
  bool resetEncoders();
      //onsemi_control::srv::ResetEncoders::Request &request,
